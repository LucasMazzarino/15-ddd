...
public class ChangeExchangeRateUseCase implements ICommandUseCase<ChangeExchangeRateRequest, Mono<TradeResponse>> {
    private final IEventsRepository repository;

    public ChangeExchangeRateUseCase(IEventsRepository repository) {
        this.repository = repository;
    }

    @Override
    public Mono<TradeResponse> execute(ChangeExchangeRateRequest request) {
        return repository.findEventsByAggregateId(request.getTradeId())
                .collectList()
                .map(events -> {
                    Trade trade = Trade.from(request.getTradeId(), events);
                    trade.exchangeRateChanged(request.getValueOrdered(), request.getValueReceived());
